...
CHECKPOINT_PATH="$REPO/checkpoints/finetune_mortality_old_pipeline/finetune_mortality_old_pipeline/best.ckpt"
...
python $REPO/training/predict.py \
  --checkpoint-path $REPO/checkpoints/finetune_mortality_old_pipeline/finetune_mortality_old_pipeline/best.ckpt \
  --config-dir $CONFIG_DIR \
  --data-dir $REPO/data \
  --sequence-file $SEQUENCE_FILE \
  --id-file $ID_FILE \
