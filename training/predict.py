...
"""Test-only script: loads a finetuned Lightning checkpoint and runs evaluation.

Bypasses all manual state_dict manipulation by using Lightning's own
load_from_checkpoint, which is exactly what trainer.test() uses internally.
"""
"""Test-only script: loads a finetuned Lightning checkpoint and runs evaluation.
...
from odyssey.data.dataset import FinetuneDatasetDecoder
from odyssey.data.tokenizer import ConceptTokenizer
from odyssey.models.ehr_mamba.model import MambaFinetune, MambaPretrain
from odyssey.models.model_utils import load_config, load_finetune_data
from odyssey.utils.utils import seed_everything
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_finetune_patients", type=str, default="100")
    parser.add_argument("--valid_scheme",     type=str, default="few_shot")
    ...
    if args.max_test_samples and len(fine_test) > args.max_test_samples:
        fine_test = fine_test.sample(n=args.max_test_samples, random_state=args.seed).reset_index(drop=True)
        print(f"[info] Subsampled to {args.max_test_samples} records.")

    # ── 2. Tokenizer ────────────────────────────────────────────────────────
    tokenizer = ConceptTokenizer(data_dir=args.vocab_dir)
    tokenizer.fit_on_vocab(with_tasks=True)

    # ── 3. Derive task name from label name ─────────────────────────────────
    task_name = (
    ...
    # ── 4. Dataset ──────────────────────────────────────────────────────────
    config = load_config(args.config_dir, "ehr_mamba")
    fine_model_config = config["model_finetune"]
    ...
    test_dataset = FinetuneDatasetDecoder(
        ...
        is_single_head=not fine_model_config.get("multi_head", False),
        additional_token_types=ADDITIONAL_TOKEN_TYPES,
        task_to_index_map={task: idx for idx, task in enumerate(tasks)},
    )
    test_loader = DataLoader(
        test_dataset,
        batch_size=args.batch_size,
    ...
    # ── 5. Build model then load checkpoint weights ─────────────────────────
    pre_model_config = config["model"]
    ...
    # Load checkpoint — Lightning's load_from_checkpoint can't be used because
    # MambaFinetune requires pretrained_model at __init__. Instead we load the
    # state dict directly, resizing vocab embeddings if needed.
    checkpoint = torch.load(args.checkpoint_path, map_location="cpu")
    ckpt_state = checkpoint.get("state_dict", checkpoint)

    vocab_resizable = {
        "pretrained_model.embeddings.word_embeddings.weight",
        "pretrained_model.model.backbone.embeddings.weight",
        "pretrained_model.model.lm_head.weight",
        "embeddings.word_embeddings.weight",
        "model.backbone.embeddings.weight",
        "model.lm_head.weight",
    }
    ...
    for k, v in ckpt_state.items():
        ...
        if v.shape == target.shape:
            final_state[k] = v
        elif k in vocab_resizable and v.ndim == 2 and v.shape[1] == target.shape[1]:
            resized = target.clone()
            n = min(v.shape[0], target.shape[0])
            resized[:n] = v[:n]
            final_state[k] = resized
            print(f"[info] Resized {k}: {tuple(v.shape)} -> {tuple(target.shape)}")

    missing, unexpected = model.load_state_dict(final_state, strict=False)
    non_vocab_missing = [k for k in missing if k not in vocab_resizable]
    if non_vocab_missing:
        print(f"[warn] {len(non_vocab_missing)} missing keys: {non_vocab_missing[:5]} ...")

    model.eval()

    # ── 6. Run test via Lightning trainer ────────────────────────────────────
    trainer = pl.Trainer(
    ...
    # ── 7. Optionally save raw predictions ───────────────────────────────────
    if args.output_dir and hasattr(model, "test_outputs"):
        import json, numpy as np
        os.makedirs(args.output_dir, exist_ok=True)
        torch.save(model.test_outputs, os.path.join(args.output_dir, "test_outputs.pt"))
        print(f"Raw predictions saved to {args.output_dir}/test_outputs.pt")
"""Run inference on a held-out dataset and compute evaluation metrics."""
